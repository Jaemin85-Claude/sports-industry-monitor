...
docs/data.json → docs/index.html (데이터 내장, 외부 의존성 없음, 모바일 우선)
"""
...
<style>
:root{--bg:#0f1420;--card:#1a2233;--line:#2a3550;--tx:#e8ecf4;--sub:#8b96ad;
--pos:#3ddc84;--neg:#ff6b6b;--accent:#4d9fff;}
...
.nm{cursor:pointer;font-weight:600}
...
.bar-wrap{flex:1;background:#0c101a;border-radius:4px;height:22px;position:relative}
.bar{height:100%;border-radius:4px;background:var(--accent)}
.bar-val{position:absolute;right:6px;top:0;line-height:22px;font-size:11px}
...
function buildSummary(){
  ...
  ['브랜드','유통'].forEach(g=>{
    ...
    DATA.items.filter(x=>x.group===g).forEach(x=>{
      ...
      h+=`<tr><td class="nm" onclick="goDetail('${x.ticker}')">${x.name}</td>
...
function buildCal(){
  ...
  const rows=DATA.items.filter(x=>x.earn_date).map(x=>{
    ...
    return {n:x.name,d,dn:Math.round((d-today)/86400000)};
  }).filter(r=>r.dn>=0&&r.dn<=90).sort((a,b)=>a.dn-b.dn);
  let h='';
  rows.forEach(r=>{
    h+=`<div class="cal-item"><div class="dn ${r.dn<=7?'hot':''}">${r.dn<=7?'🔴':'⚪'} D-${r.dn}</div>
    <div>${r.n}</div><div class="dt">${(r.d.getMonth()+1)}/${r.d.getDate()}</div></div>`;
  });
